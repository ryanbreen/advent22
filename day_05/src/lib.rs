...
pub fn part_b(input: &str) -> String {
    ...
    // Process the moves with the behavior of CrateMover 9001
    for line in lines {
        let parts: Vec<&str> = line.split_whitespace().collect();
        ...
        if let Some(crates_to_move) = stacks.get_mut(&from_stack) {
            if count <= crates_to_move.len() {
                let crates_moved: Vec<_> = crates_to_move
                    ...
                    .collect();
                if let Some(target_stack) = stacks.get_mut(&to_stack) {
                    target_stack.extend(crates_moved);
                }
            }
        }
...
mod tests {
    ...
    fn test_day_03_part_b() {
        let test_cases = [
            TestCase::<String>::new("file:inputs/part_a_1.txt", String::from("MCD")),
            TestCase::<String>::new("file:inputs/part_a_2.txt", String::from("VRQWPDSGP")),
        ];
